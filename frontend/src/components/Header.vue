...
import { addPopup, ClickMenuManager, postDataOrPopupError } from "@/assets/utils"
...
const iconSpecs = [
  {
    ...
    component: IconHome,
    uri: "/",
    tooltip: "Back to Home, open other webshells"
  },
  {
    type: "terminal",
...
const clickMenuRightClick = ClickMenuManager(
  ...
  (item) => {
    if (rightClickedIcon.uri == "/popup/no_session") {
      addPopup("red", "No WebShell selected", "Please select a WebShell on Home first")
    }
...
const clickMenuOthers = ClickMenuManager(
  ...
  (item) => {
    if (!store.session && item.link.indexOf("SESSION") != -1) {
      ...
      postDataOrPopupError("/auth/logout", {}).then(() => {
        addPopup("green", "Signed out", "You have been logged out")
        router.push("/login")
      }).catch(() => {
        addPopup("red", "Logout failed", "Unable to sign out")
      })
    }
  }
)
...
<template>
  <header :data-bg-transition="store.theme_background_transition">
    ...
    <div class="nav-space">
      ...
      <nav>
        <div class="icon" @click="historyBack()" title="Back">
          <IconLeft></IconLeft>
        </div>
        <div class="icon" @click="historyForward()" title="Forward">
          <IconRight></IconRight>
        </div>
