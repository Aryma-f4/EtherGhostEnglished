...
function onMarkDeleteSession(sessionId) {
  ...
  inputBoxTitle.value = "Delete Session"
  inputBoxNote.value = "Are you sure you want to delete this session?"
  inputBoxCallback = onDeleteSessionConfirm
}

async function onDeleteSessionConfirm(userConfirm) {
  if (!sessionToDelete) {
    addPopup("red", "Internal Error", `Cannot find the webshell to delete`)
    return
  }
  try {
    if (userConfirm) {
      ...
      if (result) {
        addPopup("green", "Deleted successfully", `Selected session deleted`)
      } else {
        addPopup("red", "Delete failed", `Unable to delete selected session`)
      }
    }
  } finally {
