...
async function doConnect() {
  if (!/\d+/.test(port.value)) {
    addPopup("red", "Invalid port", "Port is not a number")
    return
  }
  addPopup("blue", "Connection may timeout when successful", "Reverse shell blocks the current process, causing a timeout message; you can ignore it")
...
<template>
  <div class="main">
    <div class="panel shadow-box">
      ...
      <div class="panel-line">
        <input type="button" class="button" value="Connect" @click="doConnect">
      </div>
    </div>
  </div>
</template>
