...
async function onDoubleClickEntry(event) {
  ...
  if (["dir", "link-dir"].includes(entry.entryType)) {
    ...
    addPopup("red", "Unknown file type", `File ${entry.name} type unknown, cannot open`)
  }
}
...
async function submitUploadFile(form) {
  ...
  if (!resp) {
    addPopup("red", "Upload failed", "Upload failed due to unknown reasons")
  }
...
const ClickMenuFolderEntry = ClickMenuManager([
  ...
  if (item.name == "open_file") {
    ...
    addPopup("blue", "Tip", `You can double-click to open a file`)
...
function confirmNewFile() {
  ...
  inputBoxTitle.value = "New file"
  inputBoxNote.value = "Enter the file name"
  ...
  inputBoxCallback = async filename => {
    ...
    try {
      ...
      if (success) {
        addPopup("green", "Created successfully", `File ${filename} created`)
      } else {
        addPopup("red", "Creation failed", `File ${filename} creation failed`)
      }
    } finally {
      showInputBox.value = false
    }
...
function confirmRenameFile(filename) {
  showInputBox.value = true
  inputBoxTitle.value = "Rename file"
  inputBoxNote.value = `Enter new file name for ${filename}`
  inputBoxRequireInput.value = true
  inputBoxCallback = async new_filename => {
    try {
      if (new_filename) {
        ...
        if (result) {
          addPopup("green", "Renamed successfully", `File ${filename} renamed`)
        } else {
          addPopup("red", "Rename failed", `File ${filename} rename failed`)
        }
        listDir(pwd.value)
      }
    } finally {
...
function confirmDuplicateFile(filename) {
  showInputBox.value = true
  inputBoxTitle.value = "Duplicate file"
  inputBoxNote.value = `Enter new file name for ${filename}`
  inputBoxRequireInput.value = true
  inputBoxCallback = async new_filename => {
    try {
      if (new_filename) {
        ...
        if (result) {
          addPopup("green", "Duplicated successfully", `File ${filename} duplicated`)
        } else {
          addPopup("red", "Duplicate failed", `File ${filename} duplicate failed`)
        }
        listDir(pwd.value)
      }
    } finally {
...
function confirmDeleteFile(filename) {
  showInputBox.value = true
  inputBoxTitle.value = "Delete file"
  inputBoxNote.value = `Are you sure to delete file ${filename}?`
  ...
  inputBoxCallback = async result => {
    try {
      if (result) {
        ...
        if (result) {
          addPopup("green", "Deleted successfully", `File ${filename} deleted`)
        } else {
          addPopup("red", "Delete failed", `File ${filename} deletion failed`)
        }
        listDir(pwd.value)
      }
    } finally {
...
function confirmDeleteDir(filename) {
  showInputBox.value = true
  inputBoxTitle.value = "Delete folder"
  inputBoxNote.value = `Are you sure to delete folder ${filename}?`
  inputBoxRequireInput.value = false
  inputBoxCallback = async result => {
    try {
      if (result) {
        ...
        if (result) {
          addPopup("green", "Deleted successfully", `Folder ${filename} deleted`)
        } else {
          addPopup("red", "Delete failed", `Folder ${filename} deletion failed`)
        }
        listDir(pwd.value)
      }
    } finally {
...
function confirmNewDir() {
  showInputBox.value = true
  inputBoxTitle.value = "New folder"
  inputBoxNote.value = "Enter folder name"
  inputBoxRequireInput.value = true
  inputBoxCallback = async dirname => {
    ...
    try {
      ...
      if (success) {
        addPopup("green", "Created successfully", `Folder ${dirname} created`)
      } else {
        addPopup("red", "Creation failed", `Folder ${dirname} creation failed`)
      }
    } finally {
      showInputBox.value = false
    }
...
async function saveFile() {
  ...
  try {
    ...
    if (success) {
      addPopup("green", "Saved successfully", `File ${filename.value} saved`)
    } else {
      addPopup("red", "Save failed", `File ${filename.value} save failed`)
    }
  } finally {
    await listDir(pwd.value)
...
<template>
    ...
    <div class="file-content-panel">
      <div class="file-control">
        ...
        <div class="file-actions shadow-box">
          <input type="text" name="encoding" id="file-actions-encoding" placeholder="File encoding" v-model="fileEncoding">
  ...
  <transition>
    <HoverForm title="Upload File" :callback="(result) => { submitUploadFile(result); showUploadFileHoverForm = false }"
  ...
  <transition>
    <HoverStatus v-if="uploadingFiles.length + downloadingFiles.length != 0">
      <div class="file-transfer">
        <h1 v-if="uploadingFiles.length != 0">Upload Progress</h1>
        ...
        <h1 v-if="downloadingFiles.length != 0">Download Progress</h1>
