...
async def api_get_connectortype_conn_options(connector_name: str):
    """Get options for connector type"""
    if connector_name not in session_connector.session_connectors:
        raise core.UserError(f"No such Connector {connector_name}")
...
async def get_connector(connector_id: UUID):
    ...
    if not result:
        raise exceptions.UserError(f"Connector not found: {connector_id}")
    return {
        "code": 0,
